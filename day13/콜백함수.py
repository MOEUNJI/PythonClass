...
def call_function_with_name(callback,name):
    ...
    #이 함수 안에서 callback(name)을 호출해서 콜백 함수로 전달된 greeting이 실행돼요.
    #callback(name)의 의미는 callback이라는 함수에 name이라는 값을 전달해서 실행하라는 뜻.
    #예를 들어, callback에 greeting이라는 함수가 전달되었다면, 이 줄은 greeting(name)과 같아짐.
call_function_with_name(greeting,"은지")

'''
실행 과정 설명
호출 : call_function_with_name(greeting,"은지") 실행
--> call_function_with_name 함수에 greeting이라는 함수와 "은지" 라는 데이터 넣음

함수선언부 : call_function_with_name(callback,name)에서는
callback은 greeting 함수가 되고, name은 "은지"가 됨.

call_function_with_name 내부 실행
함수 내부에서 callback(name)을 실행하는데,
callback은 매개변수로 받은 greeting이므로, greeting(name)과 같다.
따라서 이 줄은 greeting("은지")로 바뀜.

greeting("은지") 실행
greeting 함수가 실행되면서, name에 "은지"가 전달됨.
greeting 함수 코드인 print(f"안녕,{name}!")이 실행되고, 안녕, 은지!라는 문장이 화면에 출력돼요.
...
divide_result = calculator(2,3,divide) # calculator호출 매개변수 2,3 연산자는 divide함수
